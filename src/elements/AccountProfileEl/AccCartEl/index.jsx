...
const AccCartEl = () => {
    ...
    useEffect(() => {
        const total = cartItems.reduce((acc, item) => acc + item.discount * item.quantity, 0);
            ...
            {cartItems.length > 0 ? (
                <>
                        ...
                        {cartItems.map((item) => (
                            <Col xs={24} sm={12} md={8} lg={6} key={item.id}>
                                <Card
                                    ...
                                    className="shadow-lg transition-transform transform hover:scale-105 flex flex-col justify-between h-full rounded-xl"
                                >
                                    ...
                                    <p className="text-sm text-gray-500 mt-2">
                                        Total: ₹{(item.discount * item.quantity).toLocaleString("en-IN")}
                                    </p>
                                </Card>
                            </Col>
                        ))}
                    ...
                    <div className="mt-6 flex flex-col sm:flex-row justify-between items-center">
                        ...
                        <button
                            className="bg-gradient-to-r from-orange-500 to-red-500 text-white font-bold py-3 px-6 rounded-lg shadow-lg transition-transform transform hover:scale-105 active:scale-95"
